...
class DecisionTransformer(TrajectoryModel):
    ...
    def __init__(
        ...
        self.predict_xygoal = torch.nn.Linear(hidden_size, 2)
        self.predict_action = nn.Sequential(
            *([nn.Linear(hidden_size, self.act_dim)] + ([nn.Tanh()] if action_tanh else []))
        )


    def forward(self, states, actions, xygoals,  timesteps, attention_mask=None):
        ...
        stacked_inputs = torch.stack(
            (xygoals_embeddings, state_embeddings, action_embeddings), dim=1
        ...
        # get predictions
        xygoal_preds = self.predict_xygoal(x[:,0])  # predict next return given state and action
        state_preds = None
        action_preds = self.predict_action(x[:,1])  # predict next action given state

        return state_preds, action_preds, None, xygoal_preds
    ...
    def get_action(self, states, actions, goal, timesteps, **kwargs):
        ...
        if self.max_length is not None:
            ...
            actions = torch.cat(
                ...
                dim=1).to(dtype=torch.float32)
            xygoal = torch.cat(
                [ torch.zeros((xygoal.shape[0], self.max_length-xygoal.shape[1], 4), device=states.device), xygoal],
                dim=1).to(dtype=torch.float32)
            timesteps = torch.cat(
